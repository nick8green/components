...
vi.mock('components/Markdown', () => {
  ...
  const Markdown = ({ children, renderers }: any) => {
    ...
    if (text.startsWith('# ') && renderers && typeof renderers.h1 === 'function') {
      nodes.push(
        renderers.h1({
    ...
    nodes.push(React.createElement('div', { 'data-testid': 'md-content', key: 'md' }, text));
    return React.createElement(React.Fragment, null, ...(nodes as ReactNode[]));
  };
...
describe('Post component', () => {
  ...
  it('shows Created by when action is create and Edited by when action is edit', () => {
    ...
    expect(screen.getByText(/Created by Bob on 3rd September 2020/)).toBeTruthy();
    ...
    expect(screen.getByText(/Edited by Bob on 3rd September 2020/)).toBeTruthy();
  });

  it('conditionally renders views when provided', () => {
    const { rerender } = render(
      <Post title="T" author="C" date={new Date('2020-09-03T00:00:00Z')} content="c" views={42} />,
    );
    expect(screen.getByText('42 views')).toBeTruthy();

    rerender(<Post title="T" author="C" date={new Date('2020-09-03T00:00:00Z')} content="c" />);
...
This is a test of the header renderer.`}
      />,
    );
    ...
    expect(heading).toHaveAttribute('id', 'hello');
  });
});

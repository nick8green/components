import Input, { InputType } from '@lib/components/Input';
...
describe('Input component', () => {
  ...
  describe('Renders as a different type correctly', () => {
    ...
    it('for a dropdown input', () => {
      ...
      render(
        <Input id="testing" onChange={() => null} options={options} type={InputType.DROPDOWN} />,
      );
  ...
  describe('Validates input correctly', () => {
    ...
    validationTests.forEach((tc) => {
      it(`for a ${tc.rule} ${tc.type} input`, () => {
        ...
        render(<Input id="testing" onChange={() => null} type={tc.type} {...rules} />);
        const input = screen.getByRole('textbox');

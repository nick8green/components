import Markdown, { flattenToId, headingRenderer } from '@lib/components/Markdown';
...
[^1]: My reference.`;
  ...
  describe('heading renderer function', () => {
    test('errors if heading tag is invalid', () => {
      expect(() => {
        headingRenderer({ children: 'test', node: { tagName: 'div' } as unknown as Element });
      }).toThrow('invalid heading tag');
    });

    test('errors if heading level is invalid', () => {
      expect(() => {
        headingRenderer({ children: 'test', node: { tagName: 'h8' } as unknown as Element });
      }).toThrow('heading level invalid! you have exceeded the maximum level of embedding');
    });
  });
});

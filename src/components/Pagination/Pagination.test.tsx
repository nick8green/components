...
vi.mock('components/Button', () => {
  return {
    default: (props: any) => {
      ...
      return (
        <button {...rest} className={className} disabled={disabled} onClick={onClick}>
          {text}
        </button>
      );
    },
...
describe('Pagination', () => {
  ...
  it('renders page info and page buttons with the current page disabled', () => {
    ...
    expect(btn2).toBeDisabled(); // current page disabled
  ...
  it('disables Previous on last page', () => {
    ...
    const prevLast = screen.getByTestId('previous');
    const nextLast = screen.getByTestId('next');
  ...
  it('throws when currentPage is out of bounds or totalPages is less than 1', () => {
    ...
    // currentPage < 1
    expect(() => render(<Pagination currentPage={0} totalPages={3} onPageChange={noop} />)).toThrow(
      'current page value is out of bounds',
    );

    // currentPage > totalPages
    expect(() => render(<Pagination currentPage={4} totalPages={3} onPageChange={noop} />)).toThrow(
      'current page value is out of bounds',
    );

    // totalPages < 1
    expect(() => render(<Pagination currentPage={1} totalPages={0} onPageChange={noop} />)).toThrow(
      'total pages must be 1 or greater',
    );
  });
});

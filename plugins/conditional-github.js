...
module.exports = {
  verifyConditions: async (pluginConfig, context) => {
    if (process.env.GITHUB_ACTIONS !== 'true') {
      context.logger.info('Skipping GitHub plugin: not running in GitHub Actions');
      return;
    }
  ...
  publish: async (pluginConfig, context) => {
    if (process.env.GITHUB_ACTIONS !== 'true') {
      context.logger.info('Skipping GitHub publish: not running in GitHub Actions');
      return;
    }
  ...
  addChannel: async (pluginConfig, context) => {
    if (process.env.GITHUB_ACTIONS !== 'true') {
      context.logger.info('Skipping GitHub channel: not running in GitHub Actions');
      return;
    }
  ...
  success: async (pluginConfig, context) => {
    if (process.env.GITHUB_ACTIONS !== 'true') {
      context.logger.info('Skipping GitHub success: not running in GitHub Actions');
      return;
    }
  ...
  fail: async (pluginConfig, context) => {
    if (process.env.GITHUB_ACTIONS !== 'true') {
      context.logger.info('Skipping GitHub fail: not running in GitHub Actions');
      return;
    }
